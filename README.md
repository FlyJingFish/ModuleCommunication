...
buildscript {
    dependencies {
        //必须项 👇
        classpath 'io.github.flyjingfish:modulecommunication-plugin:1.3.1'
    }
}
```
...
dependencies {
    //必须项 👇（可以直接放在公共 module）
    implementation 'io.github.flyjingfish:modulecommunication-annotation:1.3.1'
}
```
...
dependencies {
    //使用拦截器（不是必须的）
    implementation 'io.github.flyjingfish:modulecommunication-intercept:1.3.1'
    //使用路径的方式跳转才需要 （不是必须的）
    implementation 'io.github.flyjingfish:modulecommunication-route:1.3.1'
}
```
...
想要让 `拦截器` 和 `ModuleRoute` 起作用，您还需要使用 AndroidAOP [点此跳转查看使用方式](https://github.com/FlyJingFish/AndroidAOP),没有特别需要的话，使用里边的 debugMode 就够了,配置完之后设置下边的信息。更多路由的详细用法[点此查看](https://github.com/FlyJingFish/LightRouter#lightrouter)
